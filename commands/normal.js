...
const data = new SlashCommandBuilder()
  .setName("일반")
  .setDescription("롤 일반게임 모집을 시작합니다.");
...
async function execute(interaction) {
  ...
  try {
    ...
    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        .setCustomId("normal_set_start_time")
        .setLabel("게임 정보 입력")
    ...
    await interaction.editReply({
      content: "✅ 일반게임 모집이 성공적으로 생성되었습니다.",
    });
  } catch (err) {
...
async function handleNormalInteraction(interaction) {
  const guildId = interaction.guild?.id;
  if (!guildId) return;
  ...
  if (customId.startsWith("normal_delete_confirm:")) {
  ...
  if (customId === "normal_delete_cancel") {
    return interaction.update({
      content: "삭제가 취소되었습니다.",
      components: [],
    });
  ...
  if (interaction.isButton()) {
    if (customId === "normal_set_start_time") {
      ...
      const modal = new ModalBuilder()
        .setCustomId(`normal_modal:${msgId}`)
        .setTitle("일반게임 시작 설정");
      ...
      return interaction.reply({
        ...
        components: [
          new ActionRowBuilder().addComponents(
            new ButtonBuilder()
              .setCustomId(`normal_delete_confirm:${msgId}`)
              .setLabel("확인 ✅")
              .setStyle(ButtonStyle.Danger),
            new ButtonBuilder()
              .setCustomId("normal_delete_cancel")
              .setLabel("취소")
    ...
    if (customId === "normal_cancel") {
      removePlayer(guildLobby[msgId], userId);
    } else if (customId === "normal_substitute") {
      removePlayer(guildLobby[msgId], userId);
      if (!guildLobby[msgId].substitutes.includes(userId))
        guildLobby[msgId].substitutes.push(userId);
    } else if (customId === "normal_any") {
      removePlayer(guildLobby[msgId], userId);
      if (!guildLobby[msgId].any.includes(userId))
        guildLobby[msgId].any.push(userId);
    } else if (customId.startsWith("normal_pos_")) {
      const role = customId.replace("normal_pos_", "");
      if (guildLobby[msgId].players[role]) {
        return interaction.reply({
      ...
      removePlayer(guildLobby[msgId], userId);
      guildLobby[msgId].players[role] = userId;
    }
  ...
  if (
    ...
    customId.startsWith("normal_modal:")
  ) {
...
async function updateEmbed(msgId, interaction, partyData) {
  try {
    ...
    const embed = new EmbedBuilder()
      ...
      .setColor(isFull ? 0x95a5a6 : 0x1abc9c);
    ...
    row2.addComponents(
      new ButtonBuilder()
        .setCustomId("normal_any")
        .setLabel("상관없음 🎲")
        .setStyle(ButtonStyle.Secondary)
        .setDisabled(isFull),
      new ButtonBuilder()
        .setCustomId("normal_substitute")
        .setLabel("예비 참가")
        .setStyle(ButtonStyle.Secondary),
      new ButtonBuilder()
        .setCustomId("normal_cancel")
        .setLabel("참여 취소")
        .setStyle(ButtonStyle.Danger),
      new ButtonBuilder()
        .setCustomId("normal_delete")
        .setLabel("모집 삭제 🗑️")
        .setStyle(ButtonStyle.Danger),
    );
...
module.exports = { data, execute, handleNormalInteraction };
